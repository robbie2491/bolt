...
class SecurityPolicy implements SecurityPolicyInterface
{
    ...
    public function checkSecurity($tags, $filters, $functions)
    {
        foreach ($tags as $tag) {
            if (!in_array($tag, $this->allowedTags)) {
                throw new SecurityNotAllowedTagError(sprintf("<b>Security Policy:</b> Tag '<tt>%s</tt>' is not allowed.", $tag), $tag);
            }
        }

        foreach ($filters as $filter) {
            if (!in_array($filter, $this->allowedFilters)) {
                throw new SecurityNotAllowedFilterError(sprintf("<b>Security Policy:</b> Filter '<tt>%s</tt>' is not allowed.", $filter), $filter);
            }
        }

        foreach ($functions as $function) {
            if (!in_array($function, $this->allowedFunctions)) {
                throw new SecurityNotAllowedFunctionError(sprintf("<b>Security Policy:</b> Function '<tt>%s</tt>' is not allowed.", $function), $function);
            }
        }
    }
    ...
    public function checkMethodAllowed($obj, $method)
    {
        ...
        if (!$allowed) {
            throw new SecurityError(sprintf("<b>Security Policy:</b> Calling '<tt>%s</tt>' method on a '<tt>%s</tt>' object is not allowed.", $method, $objClass));
        }
    }
    ...
    public function checkPropertyAllowed($obj, $property)
    {
        ...
        if (!$allowed) {
            throw new SecurityError(sprintf("<b>Security Policy:</b> Calling '<tt>%s</tt>' property on a '<tt>%s</tt>' object is not allowed.", $property, $objClass));
        }
    }
